...
import SolucionPreview, { type Vista } from "../SolucionPreview";
    ...
    <section id="soluciones" className="section-y relative isolate scroll-mt-16 overflow-hidden bg-surface-alt">
      ...
      <Container>
        <SectionHeading
          ...
          intro="Todo lo que construimos sale de un proceso ya entendido y rediseñado. Estas son las piezas con las que trabajamos."
        />
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:mt-14 lg:grid-cols-4 lg:gap-5">
          {soluciones.map((s, i) => (
            ...
            >
              ...
              {/* Al pasar el mouse, el ícono deja ver una maqueta de la solución */}
              <div className="relative h-24 w-full">
                <Image
                  ...
                  height={160}
                  loading="lazy"
                  className="h-20 w-20 object-contain transition-opacity duration-300 lg:group-hover:opacity-0"
                />
                <span className="pointer-events-none absolute inset-0 hidden opacity-0 transition-opacity duration-300 group-hover:opacity-100 lg:block">
                  <SolucionPreview vista={s.vista as Vista} />
                </span>
              </div>
              ...
              {/* Herramientas: siempre visibles en celular, al pasar el mouse en escritorio */}
              <ul className="relative mt-4 flex flex-wrap gap-1.5 transition-opacity duration-300 lg:opacity-70 lg:group-hover:opacity-100">
                {s.herramientas.map((h) => (
                  <li
                    key={h}
                    className="rounded-full border border-border bg-surface-alt px-2.5 py-1 text-xs font-medium text-ink-muted"
                  >
                    {h}
                  </li>
                ))}
              ...
              <span
                aria-hidden
                className="mt-5 block h-px w-0 flow-gradient transition-[width] duration-500 ease-[var(--ease-out-soft)] group-hover:w-full"
              />
            </article>
          ))}
        </div>
